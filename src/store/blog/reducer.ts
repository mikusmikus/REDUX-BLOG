...
export const blogPostsReducer = (state = initialState, action: AllActions) => {
  switch (action.type) {
    ...
    case ADD_NEW_COMMENT: {
      ...
      const index = copyState.findIndex(({ postId }) => postId === action.comment.postId);
    ...
    case DELETE_POST: {
      ...
      const index = copyState.findIndex(({ postId }) => postId === action.postId);
    ...
    case EDIT_POST: {
      ...
      const index = copyState.findIndex(({ postId }) => postId === action.post.postId);
